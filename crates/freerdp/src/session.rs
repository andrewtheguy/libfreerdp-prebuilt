...
fn apply_settings(config: &Connect, ctx: *mut sys::rdpContext) -> Result<(), Error> {
    ...
    let bools: &[(_, bool)] = &[
        ...
        //
        // Those counts are mechanism and not volume, so whether the mechanism is *cheaper* was
        // measured separately: inbound bytes on the socket to xrdp over fixed 25-second windows
        // held by a real embedder, arms interleaved on/off/on/off so a drifting desktop cannot
        // land on one of them. A static desktop moved kilobytes either way — nothing to be better
        // at. Video in a window: 104 MB against 121 MB, about 14% less, the arms not overlapping.
        // Full-screen video, the case with a reason to regress because nearly every pixel is novel
        // and each tile now carries an order as well: 279 MB against 301 MB over four pairs, about
        // 7% less, and all six pairs across both workloads fell the same way. Read that as a byte
        // rate rather than a per-frame efficiency claim — the uncached arm painted slightly more
        // frames downstream, which this instrument cannot separate from cheaper ones.
        //
